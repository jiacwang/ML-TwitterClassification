function [ f, Xt ] = preprocess_multinomialNB( X, Y, W, pseudocount )
%PREPROCESS_MULTINOMIALNB Summary of this function goes here
...
Ny = sum(Yvec);  % weighted number of samples per label
Njy = (abs(W .* X)' * Yvec) + pseudocount;  % P x 5 number of occurences of each word in each label
...
% calculate...
sampleP_log = Plabels_log + X * Pwordlabel_log;
